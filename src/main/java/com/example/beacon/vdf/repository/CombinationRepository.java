...
public interface CombinationRepository extends JpaRepository<CombinationEntity, Long> {
    ...
    @Query(value = "SELECT * FROM combination pulse where pulse.time_stamp > ?1 order by pulse.time_stamp limit 1;", nativeQuery = true)
    CombinationEntity findNext(ZonedDateTime timeStamp);

    @Query(value = "SELECT * FROM combination pulse where pulse.time_stamp < ?1 order by pulse.time_stamp desc limit 1;", nativeQuery = true)
